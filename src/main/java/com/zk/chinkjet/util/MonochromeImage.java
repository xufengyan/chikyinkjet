...
public class MonochromeImage {
    ...
    public static File setMonchromeImage(InputStream file, String basePath) throws IOException {

        String path = basePath +"monichrome/"+ DateFormat.getNowDate()+"/";
        File fileUrl = new File(path);
        //判断当前文件夹是否存在
        if(!fileUrl.exists()){
            fileUrl.mkdirs();
        }
        String newFileName = UUID.randomUUID().toString().replace("-", "")+".bmp";
